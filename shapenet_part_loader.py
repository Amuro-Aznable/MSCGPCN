...
class PartDataset(data.Dataset):
    def __init__(self, root=dataset_path, npoints=2500, classification=False, class_choice=None, split='train', normalize=True):
        ...
        # with open(os.path.join(self.root, 'train_test_split', 'shuffled_prediction_file_list.json'), 'r') as f:
        #     prediction_ids=set([str(d.split('/')[2]) for d in json.load(f)])
        ...
        for item in self.cat:
            ...
            #     fns = [fn for fn in fns if fn[0:-4] in prediction_ids]
            else:
